...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		/*Arm*/
		model = glm::translate(modelTemp, glm::vec3(0.0f, 0.0f, 0.0f)); //We set the pivot in (0, 0, 0).
		model = glm::rotate(model, glm::radians(hombro), glm::vec3(0.0f, 0.0, 1.0f)); //Pivot around Z axis, located in (0, 0, 0).

		modelTemp = model = glm::translate(model, glm::vec3(1.5f, 0.0f, 0.0f)); //We use a temp matrix to set the center of the arm in (1.5, 0, 0). The pivot is the shoulder.
		model = glm::scale(model, glm::vec3(3.0f, 1.0f, 1.0f)); //We scale the arm.
		color = glm::vec3(0.0f, 1.0f, 0.0f); //Set the arm color to green.
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36); //Draw the arm.

		/*Forearm*/
		model = glm::translate(modelTemp, glm::vec3(1.5f, 0.0f, 0.0f)); //We set the pivot 1.5 units to the right of the temp matrix.
		model = glm::rotate(model, glm::radians(codo), glm::vec3(0.0f, 1.0, 0.0f));//The pivot will rotate around the Y axis.

		modelTemp = model = glm::translate(model, glm::vec3(1.0f, 0.0f, 0.0f));	//We set the temp matrix 1 unit next to its previous position.
		model = glm::scale(model, glm::vec3(2.0f, 1.0f, 1.0f)); //Scale the forearm.
		color = glm::vec3(1.0f, 0.0f, 0.0f); //Red color.
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);//Draw the forearm.


		/*Wrist*/
		model = glm::translate(modelTemp, glm::vec3(1.0f, 0.0f, 0.0f)); //Model is in (5,0,0). -> Pivot.
		model = glm::rotate(model, glm::radians(muneca), glm::vec3(1.0f, 0.0, 0.0f));

		modelTemp2 = modelTemp = model = glm::translate(model, glm::vec3(0.25f, 0.0f, 0.0f));//ModelTemp & ModelTemp2 are in (5.25,0,0).
